...
for i, line in enumerate(lines):
    if "fun ChatMessageBubble(" in line:
        start_idx = i - 1
    if start_idx != -1 and "fun AttachmentMenuItem(" in line:
        end_idx = i - 1
        break

if start_idx != -1 and end_idx != -1:
    new_lines = """@OptIn(androidx.compose.foundation.ExperimentalFoundationApi::class)
@Composable
                ...
                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    if (!isUser && message.voicePath != null) {
                        Row(
                            ...
                            Icon(
                                imageVector = if (isPlaying) Icons.AutoMirrored.Outlined.VolumeMute else Icons.AutoMirrored.Outlined.VolumeUp,
...
"""
    new_content = "".join(lines[:start_idx]) + new_lines + "".join(lines[end_idx:])
    with open("app/src/main/java/com/example/ui/screens/ChatScreen.kt", "w") as f:
        f.write(new_content)
    print("Replaced bubble correctly.")
else:
